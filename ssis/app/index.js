...
import {
    ...
    requestSchedule,
    cacheInterval,
    validRequests, lunchMenuDays,
    lunchMenuDayNames,
    requestToReadable
} from "../common/constants";
import {UserInterfaceHandler, View} from "./uiComponents/userInterfaceHandler";
...
function checkCacheAvailable(dataType){
    return data[dataType].savedAt !== null && Date.now() - data.lunchMenu.savedAt < cacheInterval
}
        ...
        menuListPool.delegate = {
            ...
            configureTile: (tile, info) => {
                if (info.type === "lunchMenuItemsPool"){
                    console.log(`Setting text of tile ${info.index} in pool to ${info.value}`)
...
function renderSchedule(){
    uiHandler.setActiveUserInterface("scheduleScreen", ()=>{
            ...
            getTileInfo: (index)  => {
             ...
             configureTile: (tile, info) => {
                    if (info.type === "scheduleItemsPool"){
                        const handleUnknownValue = (input) => { // Replaces any unknown values (null or empty list/object) with "--"
                            if (input === null){
                                return "--"
                            }
        ...
        console.log(`Rendered ${scheduleListPool.length} items and scrolled to the active schedule item (${activeScheduleItemTileIndex}).`)
})
}
// ...and for errors...
/**
...
function renderLoading(message){
    uiHandler.setActiveUserInterface("loadingScreen", () => {
        uiHandler.userInterfaces.loadingScreen.helperFunctions.setLoadingMessage(message)
        ...
        setTimeout(()=>{
            if (uiHandler.numberOfViewSwitches === originalNumberOfViewSwitches && uiHandler.activeView === "loadingScreen"){
                console.log("Caught loading screen timeout.")
                renderError("Timeout för att hämta data uppnåddes. Kolla att din mobil är i närheten.")
            }
        }, 15000)
    })
}
...
messaging.peerSocket.onmessage = (event) => {
    ...
    function cacheData(type, message, doneCallbacks){
        ...
        if (message.totalDataEntries === undefined || data[type].data.length === message.totalDataEntries){
    ...
    if (message.status === statusOk){
        ...
        if (message.responseType === requestSchedule){
            ...
            console.warn(`Received unknown/unparseable response type ${message.responseType} from companion!`)
