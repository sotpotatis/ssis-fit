...
const ROOMS_LOWERCASE = [
  ...
  "mastermind",
  "yatzy",
]; // A list of all the rooms in the school in lowercase.

// Regexes
const classOrClassGroupRegex = new RegExp(
  /^(ssis- *)*(te[0-9]{2}[a-z]|hela skolan|individuella val)$/,
);
...
export default class ScheduleAPI {
  ...
  getSchedule(roomName, callback) {
    ...
    fetch(url, {
      ...
      .then(function (json) {
        console.log(
          "Received response JSON from SSIS server. Converting into data...",
          json,
        );
        ...
        if (json !== null) {
          ...
          for (const event of json) {
            ...
            let parsedEvent = {
              ...
              allParticipants: [],
              isAllDay:
                eventStartDateTime.hour < 7 && eventEndDateTime.hour < 7,
            ...
            for (const participant of event.participants.split(",")) {
              const [participantType, participantText] =
