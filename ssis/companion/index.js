  ...
  if (!companion.permissions.granted("access_internet")) {
    ...
    sendMessageIfOpen(
      generateErrorMessage(
        "Behörighet för internetåtkomst inaktiverad. Slå på den i inställningar.",
      ),
    );
  }
};
...
function sendScheduleInformation() {
  ...
  if (roomNameOption === null) {
    // Validate that room is set
    console.log("No room name set. Sending error message...");
    sendMessageIfOpen(
      generateErrorMessage(
        "Ställ in din klass i inställningarna för appen i Fitbit's app.",
      ),
    );
    return;
  }
  ...
  try {
    scheduleAPI.getSchedule(roomName, function (scheduleEvents) {
      ...
      if (scheduleEvents === null) {
        ...
        sendMessageIfOpen(
          generateErrorMessage(
            "Ajdå! Ett fel inträffade när schemat skulle hämtas. Är du säker på att du valt rätt klass och att du har lektioner idag?",
          ),
        );
      }
      if (scheduleEvents.length > 0) {
        ...
        sendMessageIfOpen(
          generateErrorMessage(
            "Inga lektioner hittades för idag. Är det helg eller lov?",
          ),
        );
      }
    });
    ...
    sendMessageIfOpen(
      generateErrorMessage(`Ett fel inträffade :c Felinfo: ${e}.`),
    );
  }
}
  ...
  try {
    lunchMenuAPI.getCurrentWeekMenu(function (data) {
      ...
      if (data === null) {
        ...
        sendMessageIfOpen(
          generateErrorMessage("Ingen meny för idag hittades."),
        );
        ...
        if (Object.keys(menuDays).includes(currentDayId)) {
          ...
          console.log(
            "Day menu data retrieved: ",
            JSON.stringify(currentDayMenu),
            ". Sending...",
          );
          sendMessageIfOpen(
            generateSuccessMessage({
